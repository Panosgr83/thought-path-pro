...
export function TrustPillars() {
  return (
    <section className="reveal border-y border-border/60 bg-card">
      <div className="mx-auto grid max-w-6xl gap-px overflow-hidden bg-border md:grid-cols-3">
        {PILLARS.map((p) => (
          <div key={p.title} className="bg-card p-8 md:p-12">
            <div className="flex h-11 w-11 items-center justify-center rounded-lg bg-primary/10 text-primary">
              <p.icon className="h-5 w-5" />
            </div>
            <h3 className="mt-5 font-serif text-xl leading-snug tracking-[-0.01em] text-ink">{p.title}</h3>
